...
def detect_lane_geometry(frame):
    """
    Returns
    -------
    LaneDetectionResult
    roi_poly     : np.ndarray polygon
    car_boxes    : list of YOLO car boxes
    clean_mask   : HLS mask (cars removed)
    roi_vis      : BGR ROI edges debug
    calibrated   : bool
    """
    h, w = frame.shape[:2]
    
    # Import LaneDetectionResult here if needed, or assume it's used downstream
    from adas.perception.lane.lane_geometry import LaneDetectionResult
    ...
    # ── Dense Image Sampling ──────────────────────────────────────
    def sample_points(line, num_points=100):
        if line is None: return None, None
        x1, y1, x2, y2 = line
        if y2 == y1: return None, None
        A = (x2 - x1) / (y2 - y1)
        B = x1 - A * y1
        poly = np.array([A, B])
    ...
    left_poly, left_pts = sample_points(left)
    right_poly, right_pts = sample_points(right)
    
    pixel_count = int(np.sum(clean_mask > 0))
    
    result = LaneDetectionResult(
        left_polynomial=left_poly,
        right_polynomial=right_poly,
        left_lane_pixels=pixel_count // 2,  # Approximation
        right_lane_pixels=pixel_count // 2,
        sampled_image_points_left=left_pts,
        sampled_image_points_right=right_pts,
        confidence=100.0 if calibrated else 50.0,
        visibility=1.0,
        lane_type="UNKNOWN",
        pixel_count=pixel_count
    )

    return result, roi_poly, car_boxes, clean_mask, roi_vis, calibrated, left, right
...
def detect_lanes(frame):
    ...
    """
    ld_result, roi_poly, car_boxes, clean_mask, roi_vis, calibrated, left, right = detect_lane_geometry(frame)
