...
def ensure_bgr(img):
    """Safety net — detect_lanes already returns BGR but just in case."""
    if img is None:
...
def process_video_job(job_id: str, video_path: str):
    ...
    while True:
        ...
        # detect_lanes() returns 3 BGR frames — no cvtColor needed
        result, hls_vis, roi_vis = detect_lanes(frame)

        with jobs_lock:
            jobs[job_id]["result"]   = encode_jpeg(result)
            jobs[job_id]["edges"]    = encode_jpeg(hls_vis)  # HLS mask
            jobs[job_id]["roi"]      = encode_jpeg(roi_vis)  # ROI edges
