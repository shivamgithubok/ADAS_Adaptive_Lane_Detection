...
class Cfg:
    ...
    ROI_TOP_Y          = 0.79   # raise (e.g. 0.78) if cross still visible

    # ── HLS color mask ─────────────────────────────────────────────
    # Strategy: loose L filter + aggressive sky_cut + ROI does the
    # heavy lifting. Tight L_MAX kills lanes in sunny footage.
    WHITE_L_MIN     = 105   # picks up dim + bright lanes both
    SKY_CUT_FRAC    = 0.55  # top 50% of frame zeroed — sky+signs live here
    YELLOW_H_MIN    = 10
    YELLOW_H_MAX    = 38
    YELLOW_S_MIN    = 50
...
def hls_lane_mask(frame):
    ...
    # White lane — just needs to be bright enough
    # No upper limit — sunny lane markings can be very bright (L=240+)
    # Sky + signs are killed by sky_cut below, not by L range
    ...
    # Sky cut — zero out top N% of frame entirely
    # Signs, sky, bridge overpass all live in top half
    # Lane markings are ALWAYS in bottom half
